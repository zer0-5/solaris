...
std::vector<Point> Plane::calculateCoords() const {
    ...
    for (size_t i = 0; i < n_divisions; ++i) {
        for (size_t j = 0; j < n_divisions; ++j) {
            Point p1 = starting_point + Point(j * step, 0, i * step);
            Point p2 = starting_point + Point(j * step, 0, (i + 1) * step);
            Point p3 = starting_point + Point((j + 1) * step, 0, i * step);
            Point p4 = starting_point + Point((j + 1) * step, 0, (i + 1) * step);
