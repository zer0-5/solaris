...
std::vector<Vertex> Box::calculateCoords() const {
    ...
    for (size_t j = 0; j < n_divisions; ++j) {
        for (size_t k = 0; k < n_divisions; ++k) {
            ...
            t1 = Vec2(t_back.x() + i1.y() / 4.0f, t_back.y() + i1.x() / 3.0f);

            p2 = starting_point + length * Vec3::cartesian(0, i2.x(), i2.y());
            t2 = Vec2(t_back.x() + i2.y() / 4.0f, t_back.y() + i2.x() / 3.0f);

            p3 = starting_point + length * Vec3::cartesian(0, i3.x(), i3.y());
            t3 = Vec2(t_back.x() + i3.y() / 4.0f, t_back.y() + i3.x() / 3.0f);

            p4 = starting_point + length * Vec3::cartesian(0, i4.x(), i4.y());
            t4 = Vec2(t_back.x() + i4.y() / 4.0f, t_back.y() + i4.x() / 3.0f);
            ...
            // Front triangles
            p1.set_x(length / 2);
            t1 = Vec2(t_front.x() - i1.y() / 4.0f, t_back.y() + i1.x() / 3.0f);

            p2.set_x(length / 2);
            t2 = Vec2(t_front.x() - i2.y() / 4.0f, t_back.y() + i2.x() / 3.0f);

            p3.set_x(length / 2);
            t3 = Vec2(t_front.x() - i3.y() / 4.0f, t_back.y() + i3.x() / 3.0f);

            p4.set_x(length / 2);
            t4 = Vec2(t_front.x() - i4.y() / 4.0f, t_back.y() + i4.x() / 3.0f);
